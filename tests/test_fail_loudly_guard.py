...
in production. A stub `curl` on PATH captures (or fails) the best-effort PR
comment call so no network access is required.
"""
...
def _write_curl_stub(bin_dir: Path, log_path: Path, *, exit_code: int) -> None:
    stub = bin_dir / "curl"
    stub.write_text(
        "#!/usr/bin/env bash\n"
        f'printf \'%s\\n\' "$*" >> "{log_path}"\n'
...
def _run_guard_script(work_dir: Path, env_overrides: dict, *, curl_exit_code: int = 0):
    ...
    _write_curl_stub(bin_dir, curl_log, exit_code=curl_exit_code)
...
def test_skips_comment_when_no_pr_number_is_resolvable(tmp_path):
    ...
    assert "::error title=Vigil did not run::" in result.stdout
    assert "Vigil did not run" in summary
    # Command-specific phrasing, since resolve-addressed doesn't "post a review".
    assert "run its 'resolve-addressed' command" in result.stdout
...
def test_survives_a_failing_pr_comment_post_without_masking_the_annotation(tmp_path):
    result, output, summary, curl_calls = _run_guard_script(
        ...
        curl_exit_code=22,  # curl's --fail exit code for an HTTP error
    )
