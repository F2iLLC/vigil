...
- **Automatic issue tracking** for non-blocking observations, with severity-matched priority labels, cross-specialist merging by cited location, and open-issue deduplication.
...
- **Cross-specialist consensus** that merges duplicate findings into one comment, and observations citing the same location into one issue that quotes every specialist that raised it.
...
8. Merge duplicate cross-specialist findings into consensus findings, and cross-specialist observations citing the same location into consensus observations that name every specialist that raised them and carry each of their messages.
